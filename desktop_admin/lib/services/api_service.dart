...
class ApiService {
  ...
  Future<List<Map<String, dynamic>>> getMagasiniers() async {
    final response = await http.get(
      Uri.parse('$baseUrl/admin/magasiniers'),
      headers: await _getHeaders(),
    );
    if (response.statusCode == 200) {
      return List<Map<String, dynamic>>.from(jsonDecode(response.body));
    } else {
      throw Exception('Failed to load magasiniers');
    }
  }
  ...
  Future<void> deleteMagasinier(int id) async {
    final response = await http.delete(
      Uri.parse('$baseUrl/admin/magasiniers/$id'),
      headers: await _getHeaders(),
    );
    if (response.statusCode != 204 && response.statusCode != 200) {
      throw Exception('Failed to delete magasinier');
    }
  }
  ...
  Future<Map<String, dynamic>> createMachineGroup(String name, {int? idGroupeTech}) async {
    final response = await http.post(
      Uri.parse('$baseUrl/groupes/machines'),
      headers: await _getHeaders(),
      body: jsonEncode({
        'nom_groupe': name,
        'id_groupe_tech_principal': idGroupeTech,
      }),
    );
    if (response.statusCode == 200) {
      return jsonDecode(response.body);
  ...
  Future<void> sendCallSignal(int receiverId, String type, {bool isVideo = false}) async {
    final response = await http.post(
      Uri.parse('$baseUrl/chat/call-signal?receiver_id=$receiverId&signal_type=$type&is_video=$isVideo'),
      headers: await _getHeaders(),
    );
    if (response.statusCode != 200) {
      throw Exception('Failed to send call signal');
    }
  }
}
